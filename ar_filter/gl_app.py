...
from .metrics import head_position, halo_radius, mouth_corners, mouth_open_ratio, smooth_value
...
class ARFilterApp:
    ...
    def _draw_mouth_quads(self, centers: List[Tuple[float, float]], color: Tuple[float, float, float], scale_y: float):
        GL = self._gl
    ...
    def run(self):
        ...
        while not glfw.window_should_close(window):
            ...
            halo_r = 0.2
            halo_center = (0.0, 0.0)
            mouth_centers = [(-0.1, -0.1), (0.1, -0.1)]
            if landmarks:
                halo_r = halo_radius(landmarks)
                mouth_ratio = mouth_open_ratio(landmarks)
                halo_center = head_position(landmarks)
                mouth_centers = list(mouth_corners(landmarks))
            ...
            self._draw_mouth_quads(mouth_centers, self.mouth_color, self.mouth_scale)
